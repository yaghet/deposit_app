...
class WalletService:
    ...
    async def create_wallet(self, amount: float):
    ...
    async def perform_wallet(self, wallet_id: str, operation: OperationModel):
        async with self.session.begin():
            wallet = await self.get_wallet(wallet_id=wallet_id)

            strategy: OperationStrategyAbstract = self._strategies.get(operation.operation_type.value)
